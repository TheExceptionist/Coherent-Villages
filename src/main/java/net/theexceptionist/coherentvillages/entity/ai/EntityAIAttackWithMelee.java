...
public class EntityAIAttackWithMelee extends EntityAIBase
{
    ...
    public void updateTask()
    {
        ...
        if ((this.longMemory || this.attacker.getEntitySenses().canSee(entitylivingbase)) && this.delayCounter <= 0 && (this.targetX == 0.0D && this.targetY == 0.0D && this.targetZ == 0.0D || entitylivingbase.getDistanceSq(this.targetX, this.targetY, this.targetZ) >= 1.0D || this.attacker.getRNG().nextFloat() < 0.05F))
        {
            ...
            {
            	PolarCoord riderCoord = new PolarCoord(this.attacker.getRNG(), entitylivingbase.posX, entitylivingbase.posZ, this.attacker.posX, this.attacker.posZ);
                
            	if(phase == 0)
            	{
            		riderCoord.mirror();
            		BlockPos pos = new BlockPos(riderCoord.getWorldX(), this.attacker.world.getTopSolidOrLiquidBlock(new BlockPos(riderCoord.getWorldX(), 80, riderCoord.getWorldZ())).getY(), riderCoord.getWorldZ());
            		findPath = this.attacker.getNavigator().tryMoveToXYZ(pos.getX(), pos.getY(), pos.getZ(), this.speedTowardsTarget);
            		
            		if(findPath) phase = this.attacker.getEntityWorld().rand.nextInt(100) < 50 ? 1 : 2;
            	}
            	else if(phase == 1)
            	{
                    findPath = this.attacker.getNavigator().tryMoveToEntityLiving(entitylivingbase, this.speedTowardsTarget);
            	...
            	{	
            		riderCoord.setRandomAdjacentCoord();
            		BlockPos pos = new BlockPos(riderCoord.getWorldX(), this.attacker.world.getTopSolidOrLiquidBlock(new BlockPos(riderCoord.getWorldX(), 80, riderCoord.getWorldZ())).getY(), riderCoord.getWorldZ());
        			findPath = this.attacker.getNavigator().tryMoveToXYZ(pos.getX(), pos.getY(), pos.getZ(), this.speedTowardsTarget);
