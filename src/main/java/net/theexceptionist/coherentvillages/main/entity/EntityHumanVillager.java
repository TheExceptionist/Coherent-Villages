...
public class EntityHumanVillager extends EntityVillager implements IEntityFollower, IEntityVillager, IRangedAttackMob{
	...
	{
		{
			...
			if(this.getRNG().nextInt(100) < this.vocation.getEnchantmentChance() && meleeEnchant != null) 
			{
		...
		
		{
			...
			if(this.getRNG().nextInt(100) < this.vocation.getEnchantmentChance() && bowEnchant != null) 
			{
		...
		
		{
			...
			if(this.getRNG().nextInt(100) < this.vocation.getEnchantmentChance() && headEnchant != null) 
			{
		...
		
		{
			...
			if(this.getRNG().nextInt(100) < this.vocation.getEnchantmentChance() && chestEnchant != null) 
			{
	...
	
		{
			...
			if(this.getRNG().nextInt(100) < this.vocation.getEnchantmentChance() && legsEnchant != null) 
			{
		...
		
		{
			...
			if(this.getRNG().nextInt(100) < this.vocation.getEnchantmentChance() && bootsEnchant != null) 
			{
	...
	private PotionType getEffectiveHealing(EntityLivingBase target) {
		PotionType potion = PASSIVE_POTIONS[PASSIVE_1];
		int i = PASSIVE_1;
		...
		for(PotionType p : PASSIVE_POTIONS)
        {
	        ...
	        for (PotionEffect potioneffect : p.getEffects())
	        {
	        	...
	        	if(target.isBurning() && potioneffect.getPotion() == MobEffects.FIRE_RESISTANCE) return p;
	        	if(target.getHealth() > target.getMaxHealth() / 2 && potioneffect.getPotion() == MobEffects.REGENERATION) return p;
	        	if(target.getHealth() < target.getMaxHealth() / 2 && potioneffect.getPotion() == MobEffects.INSTANT_HEALTH) return p;
    ...
    public String getTitle()
    {
    	...
    	return villagerName+this.race.getName()+" "+this.quality.getName()+" "+this.vocation.getName();
    }
	...
	public DamageSource handleInWall(DamageSource source)
	{
		...
		else if(source.damageType.contains("fall"))
		{
			...
			{
				if(Main.allDestructive) this.handleBreakOnFall(2);
				return null;
			}
